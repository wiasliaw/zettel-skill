---
name: fleeting
description: 以蘇格拉底式思考夥伴身份發展一則原始想法或既有的 fleeting 草稿（提問攻擊面固定輪替、偶爾盲提問），經 draft-then-review 確認後寫回 staging。
...
角色：蘇格拉底式思考夥伴；寫入範圍：config `fleeting.place`；互動閘門：draft-then-review。本指令只負責發展草稿：不拆分、不歸檔（歸檔屬 `/zk:file`，擷取屬 `/zk:atomize`）、不執行任何 git 指令、不修改 plugin 檔案。
...
1. **開工前置**（依序，致命紀律）：
   ...
   d. **intent 掃描與離題捕獲**：依 `${CLAUDE_PLUGIN_ROOT}/skills/internals/intent.md` 執行（開工掃描協定、離題捕獲全程有效）；本指令的應用面：提問方向以 active intents 為據。
...
2. **輸入偵測**（`fleeting.place` 活讀 config `jq -r '.fleeting.place' .zettel.json`）：
   - `$ARGUMENTS` 是既有 `fleeting.place` 筆記路徑：就地發展該筆記，先讀入既有本文作為對話基準（本輪採盲提問時除外，見步驟 4——先提問、後讀入）。
...
4. **發展方法：蘇格拉底式提問**。以 Paul 與 Elder 的六類問題詰問這個想法，揭露其假設、證據與推論後果：
   - **釐清**：具體的意思是什麼，能否舉例。
   - **假設**：預設了什麼未經檢驗的前提。
   - **證據**：怎麼知道這件事，依據是什麼。
   - **觀點**：誰會不同意，換個立場會怎麼看。
   ...
   - **攻擊面固定輪替**：每一輪主攻擊面沿六類固定順序輪替換下一類（追問得沿線索跨類延伸，但主攻擊面不停留在同一類連續消耗）；跨回合接續時從日誌可辨的上次攻擊面接著輪替。
   - **偶爾盲提問**：目標為既有筆記時，偶爾（非每輪，由對話節奏裁量）在未讀入筆記本文的狀態下先提問一輪，之後才讀入本文對照落差——避免提問被既有內容錨定。
   - 預設立場是挑戰而非附和。
...
6. **寫回**（使用者確認後）：
   ...
   - 純文字輸入：活讀 config `fleeting.templateFile` 所指模板建立新筆記於 `fleeting.place`；`version` 活讀 config 現值（絕不用模板 placeholder 字面值）、tags 依 tagging convention、basename MUST 全 vault 筆記唯一（查名操作見 `${CLAUDE_PLUGIN_ROOT}/skills/internals/note-format.md`「命名唯一性與撞名分流」；撞名即改名消歧並重查後才落檔）。建檔同時依 devlog 協定建立其日誌（首回合 Ask 寫入使用者請求逐字）。
